...
void print_student(Student* student) {
    ...
    printf("name = %s, last name = %s\nid = %d, midterm score = %.2f, final exam score = %.2f, homework score = %.2f\n", student->first_name, student->last_name, student->student_number, student->mid_term_exam_score, student->final_exam_score, student->homework_score);
}
...
Student* search_recursive(Student* head, int id) {
    if (head == NULL)
        return NULL;
    if (head->student_number == id)
        return head;
    return search_recursive(head->next, id);
}
...
int main() {
    ...
    while (true) {
        ...
        if (!strcmp(command, "add")) {
            ...
            // Student* temp = search_recursive(head, id);
            if (!temp)
